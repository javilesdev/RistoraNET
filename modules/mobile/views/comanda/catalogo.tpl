...
<script>
    ...
    function(datos){
            $("#productos").html('');            
            for(var i = 0; i < datos.length; i++){                
                $("#productos").append('<div class="list-group-item"><div class="row"><div class="col-md-4"><img src="{$_layoutParams.root}public/img/menu/'+datos[i].imagen+'"></div><div class="col-md-5"><legend>'+datos[i].producto+'</legend></div><div class="col-md-5"><button type="button" class="btn btn-primary" data-container="body" data-toggle="popover" data-placement="bottom" data-content="Vivamus sagittis lacus vel augue laoreet rutrum faucibus." id="descripcion">Descripción</button></div><div class="col-md-1 "><h3 class="text-center">'+datos[i].precio+'</h3><p class="text-center"><button type="button" class="btn btn-primary" onclick="agregar('+datos[i].cod+');">Agregar</button></p></div></div></div>');
            }
