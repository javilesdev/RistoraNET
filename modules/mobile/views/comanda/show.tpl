...
<div class="panel-body">
	<table class="table table-condensed" style="overflow-y: auto; overflow-x: hidden; max-height: 400px;">
                    <tr>
                      ...
                      <td>
                            <button class="btn btn-warning" onclick="setlistoProd('{$mes.id_prod}','{$comanda.codTran}')" disabled >{$mes.espera}</button>
                            <button class="btn btn-info" onclick="setentregadoProd('{$mes.id_prod}','{$comanda.codTran}')" {if {$mes.retirar}==0}disabled{/if}>{$mes.retirar}</button>
                            <button class="btn btn-success" disabled>{$mes.entregado}</button>
                        </td>
                        <td>
                            <button class="btn btn-warning btn-sm" onclick="quitar('{$trans}','{$mes.id_prod}');">Quitar</button>
                        </td>
                        <td>
                            <button class="btn btn-danger btn-sm" onclick="remover('{$trans}','{$mes.id_prod}');">Borrar</button>
                        </td>
                        </tr>
                        {/foreach}         
                        {else}
...
<div class="panel-body text-center">
            ...
            <a href="{$_layoutParams.root}mobile/comanda/update_catalogo/{$comanda.ST12codMesa}/{$comanda.ST12secMesa}/{$comanda.STTcap}" class="btn btn-info btn-lg" >
                AGREGAR
            </a>
