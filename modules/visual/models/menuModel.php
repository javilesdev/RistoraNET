...
class menuModel extends Model{
    ...
    public function insertarItem($id, $prodCant, $prodPrecio, $prodTotal, $mish){
        $this->_db->query("INSERT into $mish values($id,$prodCant,$prodPrecio,$prodTotal,0);");
    }
    ...
    public function newTransact($transact, $idusuario, $codmesa, $subtotal, $iva, $total, $comentario, $seccion){
        $this->_db->prepare("INSERT INTO snvg_tt012 VALUES (:tran, now(), now(), :codusu, :codmes, :tabtran, 0, :sub, :iva, :tot, :com, :sec)")
                ->execute(
                        array(
                            ':tran' => $transact,
                            ':codusu' => $idusuario,
  ...
  STTA.SNVG_C00104=ST12.SNVG_C00101 AND
  ST12.SNVG_C00104=USU.id AND
  ST12.SNVG_C00107=0 AND ST12.SNVG_C00101=$transact ORDER BY ST12.SNVG_C00102 AND ST12.SNVG_C00103 ASC;");        
        $tran->setFetchMode(PDO::FETCH_ASSOC);
        return $tran->fetch();
    }
    ...
    public function updateItems($mish, $prodCant, $prodPrecio, $prodTotal, $id){
        $this->_db->query("update $mish set cant=$prodCant, precio=$prodPrecio, total=$prodTotal where id_prod=$id;");
    }
    ...
    public function retirar($transact){
        $this->_db->query("update $mish set cant=$prodCant, precio=$prodPrecio, total=$prodTotal where id_prod=$id;");
    }

    public function setProdListo($a1,$a2){
        $tabla = 't_' . $a1;
        $this->_db->query("update $tabla set espera=(espera-1), retirar=(retirar+1) where id_prod=$a2;"); 
    }
    ...
    public function setTransListo($a1){        
        $this->_db->query("update snvg_tt012 set SNVG_C00107=1 where SNVG_C00101=$a1;"); 
    }
